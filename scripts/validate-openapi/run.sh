...
for f in ${files[@]}; do
    ...
    npx @redocly/cli@${REDOCLY_VERSION} lint --config "$CONFIG_FILE" "$f"
done

npx @redocly/cli@${REDOCLY_VERSION} bundle --dereferenced --ext yaml --output openapi.json $(echo ${files})
